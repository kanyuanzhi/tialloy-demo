...
func (ec *EchoRouter) Handle(request tiface.IRequest) {
	...
	err := request.GetConnection().SendMsg(request.GetMsgID(), []byte("service response ECHO"))
	if err != nil {
...
func (cr *CustomRouter) Handle(request tiface.IRequest) {
	log.Printf("[Handle] messageID=%d, %s", request.GetMsgID(), request.GetData())
	err := request.GetConnection().SendMsg(request.GetMsgID(), []byte("service response CUSTOM"))
	if err != nil {
		log.Println(err.Error())
	}
}
